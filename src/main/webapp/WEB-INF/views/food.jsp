	...
	<div class="daddy">
		...
		<div class="top-outline">
			...
			<script type="text/javascript">
				...
				btn21.style.backgroundColor = 'rgb(70, 105, 167)';
				btn21.style.color = '#ffffff';
			</script>
		...
		<div class="dataList">
			<c:forEach var="dto" items="${list }">
				<div class="dataDiv">
					<div>
							<a href="${cpath }/foodInfo/${dto.fname}/">
								<img src="${cpath }/uploadFile/food/${dto.fpic}">
							</a>
					</div>
					<div class="dataName">${dto.fname }</div>
					<div class="dataAddr">
						<c:set var="faddr" value="${fn:split(dto.faddr,' ') }"></c:set>
							<c:forEach var="addr" items="${faddr }" begin="0" end="1">
								${addr }
							</c:forEach>
					</div>
				</div>
	...
	const makeToList = function(json) {
	      ...
	      for(let i = 0; i < json.length; i++){
	    	// console.log("fdfe : " + json[i].fname);
	         ...
	         div3.innerText = json[i].fname;
	         div4.innerText = json[i].faddr.split(' ')[0];
	         div4.innerText += ' '+ json[i].faddr.split(' ')[1];
	         img.src = cpath + '/uploadFile/food/' + json[i].fpic;
	         a.href = cpath + '/foodInfo/' + json[i].fname + '/';
